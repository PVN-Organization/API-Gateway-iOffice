...
public class ServiceTest {
	...
	public static void main(String args[]) {
		...
		case 4:
			...
			GetReceivedEdocResult getReceivedEdocResult = knobstickService
					.getReceivedEdocList(serviceTest.getJsonHeader());
					...
					for (Knobstick item : getReceivedEdocResult.getKnobsticks()) {
						try {
							...
							if (getEdocResult != null) {
								...
								/*
								 * File file = new
								 * File(getEdocResult.getFilePath());
								 * InputStream inputStream = new
								 * FileInputStream(file); // goi tin status
								 * if(serviceTest.getJsonHeader().matches(
								 * "status")){ Status ed =
								 * StatusXmlParser.parse(inputStream); //lay
								 * thong tin trong header MessageStatus
								 * messageHeader2 = (MessageStatus)
								 * ed.getHeader().getMessageHeader(); } //goi
								 * tin edoc else{ Ed ed =
								 * EdXmlParser.getInstance().parse(inputStream);
								 * //lay thong tin trong header MessageHeader
								 * messageHeader = (MessageHeader)
								 * ed.getHeader().getMessageHeader(); //lay
								 * thong tin file dinh kem List<Attachment>
								 * attachment = ed.getAttachments(); //lay thong
								 * tin trong body Body body = ed.getBody(); }
								 */
		...
		case 10:
			// dong bo don vi
			GetAgenciesResult getAgenciesResultQD20 = agencyService.getAgenciesListQD20(serviceTest.getJsonHeader());
			System.out.println("====>" + serviceTest.getJsonHeader());
			if(getAgenciesResultQD20!=null) {
				System.out.println(pstart+"status:"+getAgenciesResultQD20.getStatus());
				System.out.println(pstart+"Desc:"+getAgenciesResultQD20.getErrorDesc());
				System.out.println(pstart+"Size:"+getAgenciesResultQD20.getAgencies().size());
				for (int i = 0; i < 10; i++) {						
					System.out.println("_____" +getAgenciesResultQD20.getAgencies().get(i).getId() + "  __ "+ getAgenciesResultQD20.getAgencies().get(i).getCode() 
								+ "___ " + getAgenciesResultQD20.getAgencies().get(i).getName()
								+ "___ " + getAgenciesResultQD20.getAgencies().get(i).getCode102());							
				}
			}
			break;
		default:
